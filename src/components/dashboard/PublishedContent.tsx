...
import { Eye, Edit, Trash2, FileText, Image, Video, Link as LinkIcon, DollarSign } from 'lucide-react';
...
const PublishedContent: React.FC<PublishedContentProps> = ({ contents, loading, filters }) => {
  ...
  const filteredContents = filters.length > 0
  ...
  if (filteredContents.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-gray-400">No content matches your current filters</p>
      </div>
    );
  }
        ...
        return (
          <Card key={contentId} className="overflow-hidden bg-white/5 border-white/10 hover:border-emerald-500/30 transition-colors">
            <div className="p-4">
              ...
              <p className="text-xs text-gray-400 mb-3">Created on {createdAt}</p>
