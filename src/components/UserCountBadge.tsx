  ...
  return (
    <HoverCard>
      <HoverCardTrigger asChild>
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full backdrop-blur-xl bg-pastel-500/25 border border-pastel-300/50 shadow-neumorphic cursor-pointer">
          <div className="flex -space-x-2">
            {users.map((user, i) => (
              <Avatar 
                key={i} 
                className="border-2 border-white w-7 h-7 opacity-90 shadow-md"
              >
                <AvatarImage src={user.avatar} alt={user.name} />
                <AvatarFallback className="bg-pastel-100 text-pastel-700">{user.name.charAt(0)}</AvatarFallback>
              </Avatar>
            ))}
            <div className="w-7 h-7 rounded-full bg-pastel-600 flex items-center justify-center text-xs font-medium text-white border-2 border-white opacity-90 shadow-md">
              +
            </div>
          </div>
      ...
      <HoverCardContent className="w-80 backdrop-blur-xl bg-white/95 border border-pastel-200/50 shadow-neumorphic rounded-2xl">
        <div className="flex flex-col gap-3">
          ...
          <div className="space-y-3">
            {users.map((user, i) => (
              <div 
                key={i} 
                className="flex items-center gap-3 hover:bg-pastel-100/60 rounded-lg p-1.5"
              >
                <Avatar className="border border-pastel-200 shadow-sm">
                  <AvatarImage src={user.avatar} alt={user.name} />
                  <AvatarFallback className="bg-pastel-100 text-pastel-700">{user.name.charAt(0)}</AvatarFallback>
                </Avatar>
                <div>
