...
const RecentContent = ({ isAuthenticated, openAuthDialog }: RecentContentProps) => {
  ...
  const renderSkeletonCards = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {[...Array(6)].map((_, index) => (
        <Card key={`skeleton-${index}`} className="border border-border shadow-sm rounded-lg overflow-hidden">
          <CardHeader>
  ...
  return (
    <section className="py-16" id="contents">
      <div className="flex justify-between items-center mb-8">
        <h2 className="text-2xl md:text-3xl font-bold text-foreground font-lora">Recent Content</h2>
        {isAuthenticated && (
      ...
      {loading ? (
        ...
        <Card className="border border-border text-center p-8 shadow-sm">
          <p className="text-muted-foreground">No content available yet</p>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {contentArray.map((content) => (
            <Card key={content.id} className="border border-border shadow-sm rounded-lg overflow-hidden hover:shadow-md transition-shadow duration-200">
              <CardHeader>
