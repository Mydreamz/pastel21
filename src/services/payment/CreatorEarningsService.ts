...
/**
 * Service for managing creator earnings
 */
export class CreatorEarningsService {
  /**
   * Update creator's earnings total and available balance
   */
  static async updateCreatorEarnings(creatorId: string, earnings: number) {
    try {
      ...
      if (error) {
        ...
        if (error.code === 'PGRST116') {
          ...
          await CreatorEarningsService.createNewProfile(creatorId, earnings);
          return;
        }
   ...
   */
  static async createNewProfile(userId: string, initialEarnings: number) {
    try {
